...
21. javascript 创建对象的几种方式？

- 工厂模式
    就是一个工厂函数。通过在工厂函数内部创建一个对象，然后封装创建这个对象的细节，从而通过调用函数来复用创建对象的目的。有个问题是，没有建立起对象和类型间的关系。
- 构造函数模式
    优点: 可通过实例的__proto__来获取到函数原型对象，在函数原型对象中有constructor方法, 可通过 constructor来获取实例的类型。
    缺点: 每次创建的时候，都会开辟一份新的空间，对于很多相同功能的函数，造成了内存的浪费。
- 原型模式
    优点: 解决了共用函数的复用问题
    缺点: 但是方法中对对象属性的引用会造成逻辑混乱，在实际的开发中这类模式很少用。
- 组合使用构造函数和原型模式
    优点: 集合原型模式和构造函数模式的优点。
    缺点: 因为使用了两种的模式，会在编码过程中不友好，存在封装性的问题。
- 动态原型模式
    为了解决混成模式中的封装性问题,动态原型模式直接把所有的代码写在了构造函数中。
    优点: 解决了混成模式中的封装性问题。

- 寄生构造构造函数模式
    此模式和工厂模式的代码一样，唯一的区别是, 在生成实例的时候我们加上 `new`来创建。
    优点: 主要的优势是它基于一个已有的类型，在实例化的时候可以在实例上进行修改而不影响构造函数。以达到拓展对象的目的。
    缺点: 无法通过 instanceof 判断类型
- 稳妥构造函数模式
    稳妥构造函数的意思是在构造函数内部只能通过已定义的方法来get/set函数内部变量，无法在外部给函数添加方法获取和设置。
    优点: 安全
    缺点: 和寄生构造函数模式一样。
    
22. JavaScript 继承的几种实现方式？

- 第一种是以原型链的方式来实现继承。缺点是在包含有引用类型的数据时候，会被所有的实例对象共享，容易造成读写混乱。还有就是创建子类型的时候无法向超类型传递参数。
- 第二种方式借用构造函数。
- 第三种组合继承。
- 第四种原型示继承。
- 第五种寄生示继承。
